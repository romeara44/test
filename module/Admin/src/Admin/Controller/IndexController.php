...
class IndexController extends AbstractActionController
{
    public function renewalAction()
    {
        ...
        foreach ($this->getServiceLocator()->get('Admin\Model\UserTable')->getUsersByRenewalDate(time()) as $user) {
            $mail_table->sendMail($this->getServiceLocator(), array('templateKey' => 'renewaluser', 'uId' => $user->u_id));
            if ($user->u_company_id) {
                $proj_manager_role = $this->getServiceLocator()->get('Client\Model\CompanyRolesTable')->getCompanyRoleByCompanyAndRole($user->u_company_id, 8);
