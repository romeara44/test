<?php
// Filename: /module/Trainee/src/Trainee/Mapper/ZendDbSqlMapper.php
...
class ZendDbSqlMapper implements PostMapperInterface
{
   ...
   */
   public function __construct(
      ...
      TraineeUserInterface $postPrototype
   ) {
   ...
   */
   public function find($id)
   {
      ...
      throw new \InvalidArgumentException("Trainee with given ID:{$id} not found.");
   }
   ...
   */
   public function findAll()
   {
      $sql    = new Sql($this->dbAdapter);
      //$select = $sql->select('posts');
      $select = $sql->select('training_user_test');
      ...
      if ($result instanceof ResultInterface && $result->isQueryResult()) {
            ...
            $test = $resultSet->initialize($result);
            return $resultSet->initialize($result);
      }
  ...
  {
     $sql    = new Sql($this->dbAdapter);
     $select = $sql->select('companies');
     $select->where(array('c_id = ?' => $id));
