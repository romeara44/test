    ...
    public function setReportable($blId = 0)
    {
       ...
       // echo '<pre>';
        ...
        if (($questionsAnwers[1] == 2) && ($questionsAnwers[2] == 2) && ($questionsAnwers[3] == 2) && ($questionsAnwers[4] == 2) && ($questionsAnwers[5] == 1) && ($questionsAnwers[6] == 1) && ($questionsAnwers[7] == 2) && ($questionsAnwers[8] == 1)) {
            ...
            $dataBrp['brp_remediation_date'] = new \Zend\Db\Sql\Expression('NOW()');
            $dataBrp['brp_incident_date'] = new \Zend\Db\Sql\Expression('NOW()');
            ...
            foreach (Breachremediationplanaction::$tasks as $taskKey => $task) {
                ...
                if (in_array($taskKey, array(9, 10, 11, 12))) {
                    if (in_array($taskKey, array(11, 12))) {
                        ...
                        $brpaData['brpa_target_date'] = new \Zend\Db\Sql\Expression('NOW() + INTERVAL 60 DAY');
                    } else {
                        if ($blSize > 500) {
                            $brpaData['brpa_target_date'] = new \Zend\Db\Sql\Expression('NOW() + INTERVAL 60 DAY');
                        } else {
                            $brpaData['brpa_target_date'] = new \Zend\Db\Sql\Expression('NOW() + INTERVAL 356 DAY');
                        }
                    }
                } else {
                    $brpaData['brpa_target_date'] = new \Zend\Db\Sql\Expression('NOW() + INTERVAL 30 DAY');
                }
