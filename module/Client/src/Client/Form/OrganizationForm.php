...
class OrganizationForm extends Form
{
    public function __construct($sl, $c_id = 0)
    {
        ...
        $c_child_types[\Client\Model\Company::CHILD_TYPE_AS_COMPANY] = 'Treat as a Unique Organization';
        ...
        $hipaaSuiteTypes[''] = 'Please select';
        $hipaaSuiteTypesTable = $sl->get('Client\Model\HipaaSuiteTypesTable');

        foreach ($hipaaSuiteTypesTable->getHipaaSuiteTypes() as $index => $type) {
            $hipaaSuiteTypes[$type->hipaa_suite_type_id] = $type->display_description;
        }
        ...
        $this->add(array(
            'name' => 'hipaa_suite_type_id',
            'type' => 'Zend\Form\Element\Select',
            'options' => array(
                'label' => 'Hipaa Suite Type',
                'value_options' => $hipaaSuiteTypes
            ),
        ));
