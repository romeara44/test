...
class TraininglogTable implements ServiceLocatorAwareInterface
{
    ...
    public function getTraininglogs($paginated = false, $orderBy = null, $order = null, $identity = null, $searchValue = null, $roleFilter = null)
    {
        ...
        if ($paginated) {
            ...
            if ($searchValue !== null) {
                $select->where('(tl_attendees LIKE "%' . $searchValue . '%" OR rg.rg_pp_name LIKE "%' . $searchValue . '%" OR rg.rg_pp_number LIKE "%' . $searchValue . '%" OR rg.rg_number LIKE "%' . $searchValue . '%" OR tl_title LIKE "%' . $searchValue . '%" OR n.note_text LIKE "%' . $searchValue . '%")');
            }
            ...
            $select->join(array('n' => 'notes'), new \Zend\Db\Sql\Expression('tl_id = n.note_item_id AND n.note_text !="" AND n.note_item_type = ' . \Note\Model\Note::NOTE_TLC), array('_tl_comment' => new \Zend\Db\Sql\Expression('n.note_text')), 'left');
            $select->join(array('n2' => 'notes'), new \Zend\Db\Sql\Expression('tl_id = n2.note_item_id AND n2.note_text ="" AND n2.note_item_type = ' . \Note\Model\Note::NOTE_TLT), array('_tl_attachment' => new \Zend\Db\Sql\Expression('n2.note_id')), 'left');
