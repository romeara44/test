    ...
    public function editAction(){
        ...
        if ($audit_record){
            ...
            foreach ($audit_record_sections as $key => $audit_section) {
                ...
                //$audit_section->_performance_policy_numbers
                $remediationPlanNotes = [];

                $temp1 = $this->getServiceLocator()->get('Assessment\Model\NoteTable')->getRemediationplanactionsByPolicy($audit_record->rp_id, $audit_section->_performance_policy_numbers);//NOTE_RPA
                $temp = $this->getServiceLocator()->get('Note\Model\NoteTable')->getNotes(14487, \Note\Model\Note::NOTE_RPA, null, 'text');//NOTE_RPA
                foreach ($temp as $y) {
                    array_push($remediationPlanNotes, $y);
                }
